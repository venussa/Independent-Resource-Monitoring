...
<template>
    <div>
      <b-col md="12" class="header">
        <p style="margin-top: 10px;">Independent Monitoring System</p>
      ...
      <b-container class="container">
            ...
            <b-row>
              ...
              >
                ...
                <template #cell(action)="data" aria-colspan="6">
                  <div class="text-nowrap">
                    <b-link class="btn btn-primary" :to="{ name: 'detail', params: { 'n_id': $route.params.n_id, 's_id': data.item.id }}">View Detail</b-link>
                  </div>
                </template>
              ...
              <div class="col-md-6">
                ...
                <p style="margin-top: -10px">Total sensor : {{ items.length }}</p>
              </div>
              <b-col md="6">
                <b-pagination
                  v-model="currentPage"
        ...
        <div class="col-md-2">
          <i class="solar-right-icon" style="margin-top: 100%; margin-left: 30px;" />
        </div>
        </b-row>
      </b-container>
    </div>
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'updated_at', label: 'Time Create', sortable: true },
  ...
  created() {
    ...
    http.finally(() => {
      ...
      for (let x = 0; x < this.rows; x += 1) {
        const datesplit = this.items[x].updated_at.split('T')
        const getdate = this.dateConvert(datesplit[0], 1)
        const timesource = datesplit[1].split('.')
        const gettime = this.timeConvert(timesource[0], 1)
        this.items[x].updated_at = `${getdate}. ${gettime}`
      }
      // entries calculation
      this.updateState()
    })
